...
enum
{
	...
	HEADER_GC_SURVEY							= 47,
	HEADER_GC_BIOLOGIST							= 48,
	// mercenary
	HEADER_GC_MERCENARY							= 49,
	HEADER_GC_MERCENARY_EQUIP					= 50,
	HEADER_GC_MERCENARY_WARLOG					= 51,

	HEADER_GC_QUEST_GUI							= 52,
...
enum
{
	BIOLOGIST_SUBHEADER_GC_OPEN,
	BIOLOGIST_SUBHEADER_GC_CLOSE,
	BIOLOGIST_SUBHEADER_GC_UPDATE,
	BIOLOGIST_SUBHEADER_GC_REWARD,
};
...
typedef struct packet_mercenary_info
{
	char name[20];
	int race;		// 1 -> 8
	int grade;	// 1 -> 5
	...
	int leave_count;
	int level;
} TPacketMercenaryInfo;
typedef struct packet_mercenary_log
{
	char name[50];
	int kill;
} TPacketMercenaryLog;
...
{
	TEquipmentItemSet items[WEAR_MAX_NUM];
} TEquipList;
typedef struct warlog
{
	TPacketMercenaryLog log[4];
} TWarLogList;

typedef struct mercenary_list
{
	BYTE header;
	DWORD vid;
	int count;
	TPacketMercenaryInfo mercenaryList[200];
} TPacketMercenaryList;
typedef struct packet_mercenary_equip
{
	BYTE  header;
	DWORD vid;
	TEquipList equipList;
} TPacketMercenaryEquip;
typedef struct packet_mercenary_warlog
{
	BYTE  header;
	DWORD vid;
	TWarLogList warlogList;
} TPacketMercenaryWarLog;
typedef struct
{
    DWORD       dwID;
    long        x, y;
