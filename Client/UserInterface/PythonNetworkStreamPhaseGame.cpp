    ...
    while (ret)
	{
		...
		switch (header)
		{
			...
			case HEADER_GC_UNK_213: // @fixme007
				ret = RecvUnk213();
				break;
			case HEADER_GC_SURVEY:
				ret = RecvSurveyPacket();
				break;
			case HEADER_GC_BIOLOGIST:
				ret = RecvBiologistPacket();
				break;
			case HEADER_GC_MERCENARY:
				ret = RecvMercenaryPacket();
				break;
			case HEADER_GC_MERCENARY_EQUIP:
				ret = RecvMercenaryEquipPacket();
				break;
			case HEADER_GC_MERCENARY_WARLOG:
				ret = RecvMercenaryWarLogPacket();
				break;
			case HEADER_GC_QUEST_GUI:
				ret = RecvGQuestPacket();
				break;
#ifdef ENABLE_AUTO_PVP_SYSTEM
			case HEADER_GC_AUTO_PVP:
				ret = RecvAutoPvpPacket();
				break;
...
// survey
bool CPythonNetworkStream::RecvSurveyPacket()
{
	CPythonPlayer & rkPlayer = CPythonPlayer::Instance();

	TPacketSurvey survey;
...
// biologist
bool CPythonNetworkStream::RecvBiologistPacket()
{
	CPythonPlayer & rkPlayer = CPythonPlayer::Instance();
	...
	switch (info.subHeader){
		case BIOLOGIST_SUBHEADER_GC_OPEN:
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_BiologistOpen", Py_BuildValue("(ii)", info.pid_quest, info.count_item));
			break;
		case BIOLOGIST_SUBHEADER_GC_UPDATE:
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_BiologistUpdate", Py_BuildValue("(i)", info.count_item));
			break;
		case BIOLOGIST_SUBHEADER_GC_CLOSE:
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_BiologistClose", Py_BuildValue("()"));
			break;
		case BIOLOGIST_SUBHEADER_GC_REWARD:
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_BiologistReward", Py_BuildValue("()"));
			break;
		default:
			return false;
	}
...
// mercenary
bool CPythonNetworkStream::RecvMercenaryWarLogPacket()
{
	// check packet
	TPacketMercenaryWarLog mercenary_packet_warlog;
	if (!Recv(sizeof(mercenary_packet_warlog), &mercenary_packet_warlog))
	{
		Tracef("MercenaryPacket error\n");
		return false;
	}
	// check vid
	CPythonPlayer & rkPlayer = CPythonPlayer::Instance();
	if (!rkPlayer.IsMainCharacterIndex(mercenary_packet_warlog.vid))
		return false;

	TWarLogList warlogList = mercenary_packet_warlog.warlogList;

	for (int i = 0; i < 4; i++) {
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_MercenaryAddWar", Py_BuildValue("(isi)", i, warlogList.log[i].name, warlogList.log[i].kill));
	}
	return true;
}
bool CPythonNetworkStream::RecvMercenaryEquipPacket()
{
	// check packet
	TPacketMercenaryEquip mercenary_packet_equip;
	if (!Recv(sizeof(mercenary_packet_equip), &mercenary_packet_equip))
	{
		Tracef("MercenaryPacket error\n");
		return false;
	}
	// check vid
	CPythonPlayer & rkPlayer = CPythonPlayer::Instance();
	if (!rkPlayer.IsMainCharacterIndex(mercenary_packet_equip.vid))
		return false;

	TEquipList equipList = mercenary_packet_equip.equipList;
	for (int i = 0; i < WEAR_MAX_NUM; ++i)
	{

		TEquipmentItemSet & rItemSet = equipList.items[i];
		if (rItemSet.vnum == 0)
			continue;
		TraceError("MercenaryPacket %d", rItemSet.vnum);

		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_MercenarySetEquip", Py_BuildValue("(iii)", i, rItemSet.vnum, rItemSet.count));

		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_MercenarySetSocket", Py_BuildValue("(iiii)", i, rItemSet.alSockets[0], rItemSet.alSockets[1], rItemSet.alSockets[2]));

		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_MercenarySetAttr", Py_BuildValue("(iiiiiiiiiii)", i, 
																														rItemSet.aAttr[0].bType, rItemSet.aAttr[0].sValue,
																														rItemSet.aAttr[1].bType, rItemSet.aAttr[1].sValue,
																														rItemSet.aAttr[2].bType, rItemSet.aAttr[2].sValue,
																														rItemSet.aAttr[3].bType, rItemSet.aAttr[3].sValue,
																														rItemSet.aAttr[4].bType, rItemSet.aAttr[4].sValue
																													));
	}
	TraceError("Fine");
	return true;
}
bool CPythonNetworkStream::RecvMercenaryPacket()
{
	// check packet
	TPacketMercenaryList mercenary_packet;
	if (!Recv(sizeof(mercenary_packet), &mercenary_packet))
	{
		Tracef("MercenaryPacket error\n");
		return false;
	}
	// check vid
	CPythonPlayer & rkPlayer = CPythonPlayer::Instance();
	if (!rkPlayer.IsMainCharacterIndex(mercenary_packet.vid))
		return false;
	
	for (int i = 0; i < mercenary_packet.count; i++) {
		TPacketMercenaryInfo player_info = mercenary_packet.mercenaryList[i];
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_MercenaryAdd", Py_BuildValue("(isiiiiii)", i, player_info.name, player_info.race, player_info.grade, player_info.kill, player_info.war_count, player_info.leave_count, player_info.level));
	}

	//PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_MercenaryOpen", Py_BuildValue("()"));
	return true;
}
#ifdef ENABLE_AUTO_PVP_SYSTEM
bool CPythonNetworkStream::RecvAutoPvpPacket()
{
	// check packet
...
enum
{
	AUTOPVP_SEND_SUBSCRIBE,
	AUTOPVP_SEND_UNSUBSCRIBE,
	AUTOPVP_SEND_DUEL_REQUEST,
};*/
	switch (pvp_packet.subHeader)
	{
		case AUTOPVP_SEND_OPEN:
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_AutoPvPOpen", Py_BuildValue("(i)", pvp_packet.state));
			break;
		case AUTOPVP_SEND_SUBSCRIBE:
		case AUTOPVP_SEND_UNSUBSCRIBE:
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_AutoPvPState", Py_BuildValue("(i)", pvp_packet.state));
			break;
		case AUTOPVP_SEND_DUEL_REQUEST:
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_AutoPvPDuelRequest", Py_BuildValue("()"));
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_AutoPvPState", Py_BuildValue("(i)", pvp_packet.state));
			break;
		default:
			return false;
...
bool CPythonNetworkStream::RecvGQuestPacket()
{
	...
	{
		Tracef("gquestpacket error\n");
		return false;
	}
	// check vid
	CPythonPlayer & rkPlayer = CPythonPlayer::Instance();
	if (!rkPlayer.IsMainCharacterIndex(gquest.vid))
	{
	...
	switch (gquest.subHeader)
	{
		case QUEST_GUI_SUBHEADER_GC_OPEN:
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "GQuest_Open", Py_BuildValue("(s)", gquest.title));
			break;
		case QUEST_GUI_SUBHEADER_GC_ADD_TEXT:
