...
class ItemToolTip(ToolTip):
	...
	def GetItemRarity(self, itemVnum):
		if (itemVnum >= 28000 and itemVnum <=28450):
			self.idx = int((itemVnum-28000)/100)
			return constInfo.info_stone[self.idx]
		else:
			return ["", "", self.NORMAL_COLOR]

	def __SetItemTitle(self, itemVnum, metinSlot, attrSlot):
		if item.ITEM_TYPE_METIN == item.GetItemType():
			self.type_stone = self.GetItemRarity(itemVnum)
			self.AppendTextLine(str(self.type_stone[0]) + item.GetItemName(), self.type_stone[2])
			self.AppendDescription(str(self.type_stone[1]), 26)
			return
	...
	def __AppendMetinSlotInfo_AppendMetinSocketData(self, index, metinSlotData, custumAffectString="", custumAffectString2="", custumAffectString3="", leftTime=0):
		...
		## self.type_stone[name, desc, color]
		self.type_stone = self.GetItemRarity(self.itemVnum)

		## Name
		nameTextLine = ui.TextLine()
		nameTextLine.SetParent(self)
		nameTextLine.SetFontName(self.defFontName)
		nameTextLine.SetPackedFontColor(self.type_stone[2])
		...
		if itemIndex:
			...
			nameTextLine.SetText(str(self.type_stone[0]) + item.GetItemName())
			...
			if constInfo.ENABLE_ACCESSORY_SOCKET_EXPIRE:
				if 0 != leftTime:
