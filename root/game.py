...
import uiSurveyGui
import uiBiologistGui
import uiGQuestGui
import uiMercenaryGui
import uiMercenary2
import uiGQuest
import uiPvP
...
class GameWindow(ui.ScriptWindow):
	def __init__(self, stream):
		...
		self.uiGQuestGui = uiGQuestGui.GQuestGui()
		self.uiMercenaryGui = uiMercenaryGui.MercenaryGui()
		self.uiMercenary2 = uiMercenary2.MercenaryGui()
		self.uiGQuest = uiGQuest.uiGQuest()
		self.uiPvP = uiPvP.uiPvP()
		self.uiPvP_Confirm = uiPvP_Confirm.uiPvP()
		self.uiSystemList = uiSystemList.uiSystemList()
		self.uiBiologistGui = uiBiologistGui.BiologistGui()
		self.uiTeleporter = uiTeleporter.uiTeleporter()

		#self.uiBiologistGui = None
	...
	def	__PressJKey(self):
		if app.IsPressed(app.DIK_LCONTROL) or app.IsPressed(app.DIK_RCONTROL):
			...
			self.uiMercenaryGui.LoadGui()
			self.uiMercenaryGui.Show()
	...
	# Survey
	def BINARY_SurveyInfo(self, id_survey, title, text, options):
		self.uiSurveyGui = uiSurveyGui.SurveyGui()
		self.uiSurveyGui.LoadGui(id_survey, title, text, options)
		self.uiSurveyGui.Show()
		
	# Biologist
	def BINARY_BiologistOpen(self, pid_quest, count_item):
		self.uiBiologistGui.LoadGui(pid_quest, count_item)
		self.uiBiologistGui.Show()
		
	def BINARY_BiologistUpdate(self, count):
		self.uiBiologistGui.UpdateItem(count)
	def BINARY_BiologistClose(self):
		self.uiBiologistGui.Close()
	def BINARY_BiologistReward(self):
		self.uiBiologistGui.Reward()
	...
	# mercenary
	def BINARY_MercenaryAdd(self, idx, name, race, grade, kill, war_count, leave_count, level):
		self.uiMercenaryGui.AddMercenary(idx, name, race, grade, kill, war_count, leave_count, level)
		
	def BINARY_MercenaryAddWar(self, idx, name, kill):
		self.uiMercenaryGui.AddWarList(idx, name, kill)
		
	def BINARY_MercenarySetEquip(self, slotIndex, vnum, count):
		self.uiMercenaryGui.SetEquipmentDialogItem(slotIndex, vnum, count)

	def BINARY_MercenarySetSocket(self, slotIndex, value1, value2, value3):
		self.uiMercenaryGui.SetEquipmentDialogSocket(slotIndex, value1, value2, value3)

	def BINARY_MercenarySetAttr(self, slotIndex, type1, value1, type2, value2, type3, value3, type4, value4, type5, value5):
		self.uiMercenaryGui.SetEquipmentDialogAttr(slotIndex, attrIndex, type, value)

	def BINARY_AutoPvPState(self, state):
		self.uiPvP.SetSubscribe(state)
	
	def BINARY_AutoPvPDuelRequest(self):
		self.uiPvP_Confirm.LoadGui()
		self.uiPvP_Confirm.Show()
